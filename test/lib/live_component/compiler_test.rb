...
class LiveComponent::CompilerTest < ActiveSupport::TestCase
  ...
  def compile_erb_source(erb_source)
    ...
    all_ivars = LiveComponent::Compiler.extract_ivar_names(erb_ruby)
    ...
    js_body = LiveComponent::Compiler.unwrap_function(js_function, fields, raw_fields)
  ...
  test "compiles const-based each loop with extracted collection and per-item fields" do
    ...
    assert result[:expressions].values.any? { |s| s == "Label.order(:name)" },
  ...
  test "compiles bare ivar each loop with extracted collection and per-item fields" do
    ...
    assert result[:expressions].values.any? { |s| s == "@labels" },
  ...
  test "compiles const loop with ivar reference in ternary" do
    ...
    assert result[:expressions].values.any? { |s| s == "@message.labels" },
  ...
  # --- actual component compilation ---

  test "MessageLabelsComponent compiles without error" do
    data = MessageLabelsComponent.compiled_data
    assert data[:js_body].present?
    assert data[:expressions].present?
  end

  test "MessageLabelsComponent extracts labels as collection" do
    data = MessageLabelsComponent.compiled_data
    assert data[:expressions].values.any? { |s| s.include?("labels") },
      "Expected a labels-based expression, got: #{data[:expressions]}"
    assert_not_empty data[:collection_computed],
      "Expected collection_computed for labels loop"
  end

  test "MessageLabelsComponent JS does not contain raw Ruby constants" do
    data = MessageLabelsComponent.compiled_data
    assert_no_match(/Label\.order/, data[:js_body])
    assert_no_match(/LabelBadgeComponent/, data[:js_body])
  end

  test "MessageRowComponent compiles without error" do
    data = MessageRowComponent.compiled_data
    assert data[:js_body].present?
    assert data[:expressions].present?
  end
end
